...
//프로그램에서만 사용하는 구조체
typedef struct _FILE_NODE {     //파일 목록 연결 ㄹ리스트 구조체 정의
...
int append(PARCHIVE archive, char *filename)    //파일 추가 함수 정의
{
    ...
    Error1:
        ...
        return ret;     //성공이냐 실패냐에 따라 0또는 -1을 반환
}
...
int main()
{
    ...
    if (fp == NULL)                         //아카이브 파일이 없으면
    {
        ...
        if (fp == NULL)                     //아카이브 생성(열기)에 실패하면
            return -1;                      //프로그램 종료

        //새 아카이브 헤더 생성
        archive->header.magic = 'AF';   //매직 넘버 AF 저장 (리틀엔디언에서는 FA로 저장됨)
    ...
    while (size > currPos)  //파일 포인터의 위치가 파일 크기보다 작을 때 반복
    {
        ...
        if (fread(&node -> desc, sizeof(FILE_DESC), 1, fp) < 1)
        {
            ...
            free(node);     //동적 메모리 해제
            ret = -1;       //-1은 실패
            goto FINALIZE;  //모든 동적 메몰리 해제 코드로 이동
        }
